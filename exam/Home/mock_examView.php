...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['RequestMockExam'])) {
        $GId = $_POST["ReqestGrade"];
        $SubId = $_POST["RequsestSub"];
...
<body>
 <?php 
    $selExam = $conn->query("SELECT * FROM questions WHERE  GID = $GId AND SubID = $SubId ")->fetch(PDO::FETCH_ASSOC);
    $selExamTimeLimit = 10;
    $exDisplayLimit = 10;
 ?>
...
<div class="app-main__inner">
    ...
    <div class="col-md-12 p-0 mb-4">
        <form method="post" action="mock_exam_result.php" id="submitAnswerFrm">
            <input type="hidden" name="exam_id" id="exam_id" value="<?php echo 'Grade 6 Maths'; ?>">
            <input type="hidden" name="examAction" id="examAction" >
        <table class="align-middle mb-0 table table-borderless table-striped table-hover" id="tableList">
        <?php 
            $selQuest = $conn->query("SELECT * FROM questions WHERE SubID = $SubId ORDER BY rand() LIMIT $NoOfQuestions ");
